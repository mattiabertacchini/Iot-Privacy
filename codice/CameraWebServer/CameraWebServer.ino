...
// Setup LED FLash if LED pin is defined in camera_pins.h
  ...
  while (!mqtt_client.connected()) {
      ...
      //if (mqtt_client.connect(client_id.c_str(), broker_username, broker_pswd )) {
  ...
  const char* topic = "BertacchiniPanseraRistori/ip_refs/camera/A0001";
  WiFi.localIP().toString().toCharArray(ip_char_array, 255);
  int status = mqtt_client.publish(topic, ip_char_array);
